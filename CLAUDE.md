...
- `daigest-version.json` — **Source of truth** for the current dAIgest app version. The mobile app fetches this on startup to check for updates. When releasing a new version, update `version`, `versionCode`, `releaseDate`, and `changelog` here.
- `daigest.html` — dAIgest product/download page. Shows version and download count (from GitHub Releases API, cached in localStorage).
...
## Release Workflow (dAIgest)

When shipping a new dAIgest APK:

1. Build the APK in DigestEngine repo
2. Create a GitHub Release on this repo with the APK attached
3. Update `daigest-version.json` with the new version, versionCode, date, and changelog
...
- **Stripe** — dAIgest Pro checkout ($9.99 one-time)
- **GitHub Releases API** — download counter on daigest.html
